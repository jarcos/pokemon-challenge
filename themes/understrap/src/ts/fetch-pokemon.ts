interface Pokemon {
  ...
  attacks: string[];
  weight: number;
  oldVersionPokedexNumber: number;
  newVersionPokedexNumber: number;
  ajax_object: any;
}
...
interface PokemonSpecies {
  pokedex_numbers: { pokedex: { name: string }; entry_number: number }[];
}
...
declare const ajax_object: AjaxObject;

// Check if #fetchPokemonsBtn exists
if (document.querySelector('#fetchPokemonsBtn')) {
  document.querySelector<HTMLButtonElement>('#fetchPokemonsBtn').addEventListener('click', async (event) => {
    const fetchBtn = event.currentTarget as HTMLButtonElement;
    ...
    try {
      ...
      (document.querySelector('#pokemonList') as HTMLElement).style.display = "table";
...
async function fetchPokemonAttacks(pokemonId: number): Promise<Attacks[]> {
  const response = await fetch(`https://pokeapi.co/api/v2/pokemon/${pokemonId}`);
  const data = await response.json();
  const attackPromises = data.moves.map(async (move: any) => {
    const attackResponse = await fetch(move.move.url);
    const attackData = await attackResponse.json();
    const name = attackData.name;
    const description = attackData.effect_entries.find((entry: any) => entry.language.name === 'en')?.short_effect || '';
    return {
      name,
      description,
    };
...
async function getPokedexNumbers(pokemonId: number): Promise<{ oldPokedexNumber: number; newPokedexNumber: number }> {
  const response = await fetch(`https://pokeapi.co/api/v2/pokemon-species/${pokemonId}`);
